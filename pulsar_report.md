...
- In this application, the autocorrelation ratio can reveal periodicity, since strongly periodic signals produce pronounced peaks in autocorrelation, raising the maximum relative to the median background level.
...
- The **Fourier Transform** displays peaks of a signal where the signal has frequencies present. The maximum peak within the positive frequencies divided by the median magnitude results in the FFT Ratio. The FFT Ratio would reveal the level of periodicity at the most common frequency in the signal.
...
- Spectral fingerprinting successfully revealed leakage from pulsar periodicity. Both the FFT ratio and PSD ratio increased significantly after scrambling, confirming that the periodic structure remained detectable in the frequency domain. As the scrambling was altered within the time domain using linear transformations, scrambling has little effect on the FFT and PSD ratios.

- Noise, unsurprisingly, significantly reduced peak contrast for spectral fingerprinting, which reduced leakage metrics.
...
- **Enveloping** is a process that smooths signals and outlines their extremes.
...
1. **Seeded RNGs**:
   ...
   - Mersenne Twister generators build an internal state array initialized from an input seed. Each successive value in the PRNG stream is deterministically derived from this state. As a result, **re-initializing the generator with the same seed reproduces the same sequence**, provided that random draw functions are called in the same order.
...
- Although scrambling did not fully mask periodicity, it slightly reduced spectral detection strength, indicating that the method could still serve as a lightweight obfuscation layer in noisy environments.
...
5. **Spectral Ratio Scoring**: 
    ...
    - Ratios were scaled logarithmically to normalize them.
...
6. **Noise Thresholding**: 
    ...
    - Applied fast pre-filters:
		...
		- If the envelope, FFT, or PSD values were outside observed sane ranges, the seed was skipped or rejected.
	
	- This reduced false positives by ensuring a seed scored well across all tests and slightly improved runtime.
...
Autocorrelation analysis aims to detect periodicity, but in sparse signals such as the pulsars analyzed, autocorrelation performs poorly and produces unstable metrics. This means it may fail to reveal meaningful leakage. On the other hand, frequency-domain techniques were extremely effective in this application due to the periodic nature of these signals. By examining the Fourier transform and power spectral density, it was possible to identify dominant frequency components that would often persist after time-based scrambling. This would allow attackers to detect leakage if time-domain methods fail.

Lastly, by computing the analytic signal of a pulsar-like waveform using the Hilbert transform and obtaining the signal’s envelope, amplitude variations were highlighted over time. The envelope essentially smoothed out the signal, which improved metrics of other signal analysis techniques within this application.
...
This is where good circuit design comes into play as a defense against SCAs. By reducing ambient SNR for side channels, there is a reduced risk of exposing sensitive information on devices.
...
For a single-threaded process, brute-forcing 2^16 seeds took ~40 minutes. Not bad for a confused undergraduate. **Scaling to 2^128 seeds, however, would take ~0.4 nonillion years**. That's kind of a long time. Even if this system were parallelized, that would still be a *computationally infeasible* scale. 
